...
function App() {
    ...
    if (savedSession) {
      try {
        ...
        if (
          ...
          // Reset loading states to safe stable states on restore
          let restoredState = parsed.currentState || "initial";
          if (restoredState === "uploading") {
            restoredState = "initial";
          } else if (restoredState === "generating") {
            restoredState = "target_set";
          }
          ...
          const hasWelcomeBack = parsed.messages.some((m) =>
  ...
  useEffect(() => {
    if (messages.length > 1) {
      // Don't save intermediate/loading states in localStorage as "uploading" or "generating"
      let savedState = currentState;
      if (savedState === "uploading") {
        savedState = "initial";
      } else if (savedState === "generating") {
        savedState = "target_set";
      }
...
`;
  ...
  const saveCurrentSession = () => {
    ...
    const datasetName = uploadMessage
      ? uploadMessage.text.replace("📄 Uploaded: ", "").trim()
      : "Unknown Dataset";
    ...
    const existing = JSON.parse(
      localStorage.getItem("aiDataScientist_savedSessions") || "[]"
    );
  ...
  const loadSampleDatasets = async () => {
    try {
      const response = await fetch(`${API_URL}/api/sample-datasets`);
      const data = await response.json();
      setSampleDatasets(data.samples || []);
      setShowSampleDatasets(true);
    } catch (error) {
      console.error("Error loading sample datasets:", error);
      addMessage("agent", "❌ Could not load sample datasets");
    }
  };

  const handleSampleDatasetLoad = async (sampleId) => {
    addMessage("user", `📊 Loading sample dataset: ${sampleId}`);
    setIsLoading(true);
    setShowSampleDatasets(false);
    setModelComparison(null);

    try {
      const response = await fetch(`${API_URL}/api/load-sample/${sampleId}`, {
        method: "POST",
      });
      ...
      if (response.ok) {
        ...
        if (data.analysis && data.state === "analyzed") {
          ...
          addMessage("agent", data.message || "✅ Sample dataset loaded!");
        }
      } else {
        addMessage(
          "agent",
          `❌ Error: ${data.error || "Failed to load sample"}`
        );
      }
    } catch (error) {
      addMessage("agent", "❌ Network Error: Could not load sample dataset");
  ...
  const compareModels = async () => {
    ...
    addMessage(
      "agent",
      "🤖 Comparing 5 different ML models... This may take a minute."
    );
  ...
  const renderAnalysisCard = (analysisData) => {
    ...
    return (
      <div className="bg-slate-50 rounded-xl p-4 mt-2 border border-slate-200 shadow-sm">
        <div className="flex items-center mb-3">
          <BarChart3 className="text-[#1e3a8a] text-xl mr-2" />
            ...
            <div className="mt-3 p-2 bg-slate-100 rounded-lg border border-slate-200">
              ...
              <div className="flex flex-wrap gap-1">
                {analysisData.potential_targets.slice(0, 5).map((col, idx) => (
                  <span
                    key={idx}
                    className="bg-slate-200 text-slate-800 text-xs px-2 py-1 rounded-full border border-slate-300"
                  >
                    {col}
                  </span>
                ))}
  ...
  const renderPreviewCard = (previewData) => {
    ...
    return (
      <div className="bg-slate-50 rounded-xl p-4 mt-2 border border-slate-200 shadow-sm">
        <div className="flex items-center mb-3">
          <Brain className="text-[#1e3a8a] text-xl mr-2" />
          <h3 className="font-bold text-gray-800">Model Preview</h3>
        </div>

        <div className="grid grid-cols-3 gap-3 mb-3">
          <div className="bg-white rounded-lg p-3 shadow-sm text-center border border-slate-100">
  ...
  const renderModelComparisonCard = (comparisonData) => {
    ...
    return (
      <div className="bg-slate-50 rounded-xl p-4 mt-2 border border-slate-200 shadow-sm">
        ...
        <div className="bg-white rounded-lg p-3 shadow-sm mb-3 border border-slate-100">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white rounded-lg p-2 shadow-sm text-center border border-slate-100">
    ...
    return (
      <div className="bg-slate-50 rounded-xl p-4 mt-2 border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <CheckCircle className="text-emerald-500 text-2xl mr-3" />
            <div>
          ...
          <a
            ...
            className="bg-[#1e3a8a] hover:bg-[#0f1e36] text-white font-semibold py-2 px-4 rounded-lg flex items-center transition-all shadow-md hover:shadow-lg"
          >
    ...
    return (
      <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 mb-4">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
          <div
            className="bg-[#1e3a8a] h-3 rounded-full transition-all duration-500 ease-out"
    ...
    if (isLoading) {
      return (
        <div className="space-y-3">
          {renderProgressBar()}
          <div className="flex items-center justify-center p-4 bg-gray-50 rounded-xl border border-slate-200">
            <Loader2 className="animate-spin text-[#1e3a8a] text-xl mr-3" />
      ...
      return (
        ...
        >
          <input
            ...
            accept=".csv,.xlsx,.xls"
          />
          <Upload className="text-5xl text-slate-400 mx-auto mb-3" size={48} />
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <button
              ...
              className="text-[#1e3a8a] hover:text-[#0f1e36] font-medium text-sm flex items-center justify-center gap-2 mx-auto transition-colors"
            >
              <span>📊</span> Or try a sample dataset
            </button>
          </div>
        </div>
      );
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap justify-center gap-2 max-h-24 overflow-y-auto p-1">
                {analysis.columns.map((col) => (
                  <button
                    key={col}
                    onClick={() => handleSendMessage(col)}
                    className="bg-white hover:bg-slate-50 text-[#1e3a8a] border-2 border-slate-200 hover:border-[#1e3a8a] font-semibold py-1.5 px-4 rounded-lg shadow-sm transition-all transform hover:scale-105 text-sm"
                  >
                    {col}
                  </button>
                ))}
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Or type a column name..."
              className="flex-grow border-2 border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-[#1e3a8a] transition-all"
            />
            <button
              onClick={() => handleSendMessage()}
              disabled={!userInput.trim()}
              className="bg-[#1e3a8a] hover:bg-[#0f1e36] disabled:bg-gray-400 text-white font-bold py-3 px-6 rounded-xl transition-all shadow-md hover:shadow-lg disabled:cursor-not-allowed"
            >
              <Send size={20} />
            </button>
          </div>
          ...
          {choices.length > 0 && (
            <div className="space-y-3">
              <div className="flex justify-center gap-3">
                {choices.map((choice) => (
                  <button
                    ...
                    className="bg-[#1e3a8a] hover:bg-[#0f1e36] text-white font-semibold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transition-all transform hover:scale-105"
                  >
                    {choice.charAt(0).toUpperCase() + choice.slice(1)}
                  </button>
                ))}
              ...
              <div className="flex justify-center">
                {modelComparison ? (
                  <div className="bg-emerald-600 text-white font-semibold py-2 px-6 rounded-lg shadow-md flex items-center border border-emerald-700">
                  ...
                  <button
                    ...
                    className="bg-slate-700 hover:bg-slate-800 disabled:bg-gray-400 text-white font-semibold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transition-all transform hover:scale-105 disabled:cursor-not-allowed flex items-center"
                  >
                    {isComparing ? (
                      <>
      ...
      return (
        <div className="bg-green-50 border-2 border-green-200 rounded-xl p-4 text-center">
          ...
          <button
            ...
            className="mt-3 bg-[#1e3a8a] hover:bg-[#0f1e36] text-white font-semibold py-2 px-4 rounded-lg transition-all"
          >
            Start New Analysis
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      ...
      <div
        ...
        style={{ height: "90vh" }}
      >
        <div className="lg:w-1/3 bg-[#0f1e36] p-8 text-white flex flex-col">
          <div className="flex items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">AI Data Scientist</h1>
          ...
          <div className="space-y-4 text-sm mb-8">
            <div className="flex items-start">
              ...
              <div>
                ...
                <div className="text-xs text-slate-300">CSV or Excel file</div>
              </div>
            </div>
          ...
          <div className="mt-auto space-y-3">
            {sessionId && (
              <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                ...
                <div className="font-mono text-[10px] opacity-75 truncate text-slate-300">
            ...
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              <div className="text-slate-300 text-xs font-semibold mb-3 text-center">
                📊 Session Manager
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="bg-emerald-600/20 hover:bg-emerald-600/30 disabled:bg-gray-500/10 disabled:text-gray-400 text-emerald-200 text-[10px] font-semibold py-2 px-2 rounded-lg transition-all flex flex-col items-center justify-center disabled:cursor-not-allowed"
                ...
                <button
                  ...
                  className="bg-blue-600/20 hover:bg-blue-600/30 text-blue-200 text-[10px] font-semibold py-2 px-2 rounded-lg transition-all flex flex-col items-center justify-center"
                ...
                <button
                  ...
                  className="bg-white/10 hover:bg-white/20 disabled:bg-gray-500/10 disabled:text-gray-400 text-white text-[10px] font-semibold py-2 px-2 rounded-lg transition-all flex flex-col items-center justify-center disabled:cursor-not-allowed"
            ...
            <div className="flex items-center justify-between">
              ...
              {analysis && (
                <div className="flex gap-2 text-xs">
                  <div className="bg-slate-100 px-3 py-1 rounded-full text-slate-700 font-semibold border border-slate-200">
      ...
      {showSessionHistory && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col">
            <div className="bg-[#0f1e36] text-white p-6 flex items-center justify-between border-b border-slate-700">
              <div>
                ...
                <p className="text-sm text-slate-300 mt-1">
                  Your saved data science sessions
                </p>
              </div>
              <button
              ...
              {savedSessions.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-sm text-gray-500">
                    Save your current analysis to build your history!
                  </p>
                </div>
              ) : (
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="bg-[#1e3a8a] hover:bg-[#0f1e36] text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all shadow-md hover:shadow-lg"
                          >
                            📂 Restore
                          </button>
                          <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              ...
              <button
                onClick={() => setShowSampleDatasets(false)}
            ...
            <div className="p-6 space-y-4">
              {sampleDatasets.map((sample) => (
                <div
                  key={sample.id}
                  onClick={() => handleSampleDatasetLoad(sample.id)}
                  className="border-2 border-gray-200 rounded-xl p-4 hover:border-[#1e3a8a] hover:bg-blue-50/50 cursor-pointer transition-all"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-4 mt-2 text-xs text-gray-500">
                        <span>🎯 Target: {sample.target}</span>
